...
organizes them (e.g. `s/` and `ns/`) on a split drive (ADR-0017). ComfyUI lists each file *relative
to the root it was found under*, so the same file reads as `sd_xl_base_1.0.safetensors` when it sits
directly under a root and `s/sd_xl_base_1.0.safetensors` when it sits in a subfolder of one.
...
2. **The default checkpoint** (`sd_xl_base_1.0.safetensors`, baked into the workflow templates)
   failed to load, because ComfyUI only had `s/sd_xl_base_1.0.safetensors`.
...
"Installed" list printed ComfyUI's raw names (`s/…`, `ns/…`) verbatim, which *looked* like folder
...
- **`src/checkpoints.ts`** gains three pure helpers:
  ...
  - `reconcileCheckpoint(desired, available)` — maps a bare name to the exact ckpt_name ComfyUI
    expects: exact match wins; else the first entry sharing the basename (recovering a prefix like
    `s/`); else the desired value unchanged, so a truly absent model still yields ComfyUI's own clean
...
- **Operator step to see the grouping:** ComfyUI must *report* subfolders, i.e. point the
  ...
  prefixes `s/…`, `ns/…`), not at the leaf subfolders. Restart ComfyUI. With this code deployed,
