# ADR-0016: Popular models from Civitai
...
extends the catalog with a curated set of popular, verified Civitai checkpoints and style
LoRAs.

## Decision

### Curated, version-pinned
Every entry was verified live against the Civitai REST API (`/api/v1/models`): a clean content rating at the
...
Pony/Illustrious/Flux/SD-1.5 models were excluded (SDXL-only). One
candidate (Copax TimeLessXL) was **dropped** after the API showed its current version flipped to a restricted rating; Copax Melodies XL was used as the clean substitute.
...
## Consequences
- **A much wider, genuinely popular model set** is selectable by friendly name — the models people
...
- **Civitai versions can drift.** A model can later flip to a restricted rating or a version can be unpublished. The
