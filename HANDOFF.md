...
## Current state
**IP-Adapter conditions identity, not composition (ADR-0005).** `applyIPAdapter` injected the
...
### Prior — selectable base checkpoint (merged, PR #16)
...
- Docs: [docs/developer-reference.md](docs/developer-reference.md) (config field, `POST /generate`
  ...
  work above landed after it and was rebased on top (its ADR renumbered 0004 -> 0005).
...
ADR-0005 (identity-only conditioning).
