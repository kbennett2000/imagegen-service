...
test("subfoldered install -> template default reconciled to the prefixed ckpt_name (ADR-0019)", async () => {
  ...
  const mock = new MockComfy({ checkpoints: ["sub/sd_xl_base_1.0.safetensors", "sub/sd_xl_refiner_1.0.safetensors"] });
  await generateImage(URL, { prompt: "x" }, mock.fetch);
  assert.equal(mock.submitted[0]!.graph["4"].inputs.ckpt_name, "sub/sd_xl_base_1.0.safetensors");
});

test("subfoldered install -> a bare override is reconciled to the prefixed name, refiner too (ADR-0019)", async () => {
  const mock = new MockComfy({ checkpoints: ["alt/other.safetensors", "sub/sd_xl_refiner_1.0.safetensors"] });
  await generateImage(URL, { prompt: "x", quality: "high", checkpoint: "other.safetensors" }, mock.fetch);
  const graph = mock.submitted[0]!.graph;
  assert.equal(graph["4"].inputs.ckpt_name, "alt/other.safetensors"); // base override -> prefixed
  assert.equal(graph["11"].inputs.ckpt_name, "sub/sd_xl_refiner_1.0.safetensors"); // refiner -> prefixed
});
