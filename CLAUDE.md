...
A standalone TypeScript/Node service (run via tsx, like Chronicle) that fronts a local ComfyUI instance. It is a CLEAN-ROOM EXTRACTION of Chronicle's local image backend — it does NOT import from or depend on Chronicle. It keeps the reusable engine (quality-tier→workflow selection, style→LoRA recipe + trigger injection, the ComfyUI POST /prompt → poll /history → fetch /view transport, never-throw + timeout discipline) and DROPS the two Chronicle-specific concerns: prompt CONSTRUCTION (caption/grounding is the caller's job) and SAVING to disk (the service returns raw bytes; callers save).

Stack: TypeScript on Node, run via `tsx`. Node built-in `http` server + built-in `fetch`. NO web framework. NO new runtime deps beyond tsx/typescript/@types. File-based config only — NO environment variables anywhere (`grep -rn "process.env" src/` must be empty), mirroring Chronicle's config approach.

Test command: `npm run test:unit` (node --import tsx --test). All tests CI-safe: mock ComfyUI HTTP — no GPU, no running ComfyUI required.

Conventions: never throw out of a request handler (return a JSON error + appropriate status; never leak a stack); tier-aware timeouts; ADR-first for design decisions; small reviewable commits.

Reference material lives in the sibling Chronicle repo at ~/Desktop/projects/chronicle — READ these to reimplement, do NOT import: src/image-backends/local.ts (ComfyUI dance, LoRA node injection/applyLora, tier→workflow selection, never-throw/timeout), src/image-backends/style-loras.ts (the style→{loraFile,trigger,strength,noRefiner} recipe map — reuse the values verbatim), src/workflows/*.json (COPY sdxl-txt2img.json + the refiner variant into this repo's src/workflows/), src/config.ts (file-config pattern to mirror).

Build the service per docs/adr/0001-imagegen-service-spec.md (included below). First implementation slice = scaffold + config + extracted engine + /generate, /styles, /health, with the concurrency requirement. Deployment (systemd, optional auth token, expanded README) is a deliberate second slice — do not fold it in.
